...
The canonical release bundle: Chrome extension + optional local CV backend. The `extension/` alone does both features with no Python — crime stats and room square-footage both run on-device (the room-size model runs in the browser via WebGPU/WASM). The optional backend here adds a higher-accuracy **multi-photo** room-size mode (DUSt3R camera fusion).

## Requirements (optional backend only)
