...
- `scripts/compile-data.js` generates:
  - `selfhost-nc/extension/data/crime-stats.json` (currently only `24m`)
  - `selfhost-nc/extension/data/nta-boundaries.json`
...
- Ambient risk index denominator uses resident population + LODES WAC jobs (see `computeAmbientPopulation` in `selfhost-nc/extension/lib/utils.js`).
- `scripts/compile-nta-exposure.py` generates `selfhost-nc/extension/data/nta-exposure.json` and expects local inputs under `../data/` (Census + LODES parquet). If Python deps are missing, install via `uv pip install ...` (no bare `pip`).
