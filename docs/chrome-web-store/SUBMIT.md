...
- **Upload package:** `sleepeasy-chrome-store-v2.1.0.zip` — built by `scripts/build-extension.sh` into `dist/`, also copied to `~/Downloads`. Flat zip (manifest at the root).
...
3. **+ New item** → upload `sleepeasy-chrome-store-v2.1.0.zip`.
