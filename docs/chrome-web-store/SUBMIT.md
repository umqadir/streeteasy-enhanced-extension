...
The store build runs both features on-device (crime stats + in-browser photo
area estimates). No localhost permission.

## What's prepared

- **Upload package:** `sleepeasy-chrome-store-v2.2.2.zip`, built by `scripts/build-extension.sh` into `dist/` and copied to `~/Downloads`. Flat zip, with the manifest at the root.
...
3. **Package**: upload `sleepeasy-chrome-store-v2.2.2.zip`.
...
- **"Why fetch from huggingface.co?"** The photo-area feature downloads the open-source CV model **weights** (data files) once, then caches them. No executable code is fetched; the JS/WASM runtime is in the package. No user data is sent. This is the standard pattern for local ML extensions.
- **"Why access the photo CDN?"** To fetch the listing's own photos so the model can measure floor area in the user's browser. The images are not stored or transmitted.
...
- **Web Store** (this doc): crime stats + local single-photo room area estimates. No backend, no localhost.
- **GitHub:** `Load unpacked` build can use the optional local Python backend for single-photo estimates. It keeps a `localhost` permission and can't ship through the Web Store because it talks to a local process.
