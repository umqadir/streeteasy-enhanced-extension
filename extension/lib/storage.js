 ...
 *   "area:scans:{listingId}"  -> ScanResult[]
 */
...
class ListingStorage {
  ...
  _scansKey(listingId) { return `area:scans:${listingId}`; }
  ...
  async deleteListing(listingId) {
    ...
    await this._remove(this._scansKey(listingId));
  }
  ...
  /** @param {string} listingId @returns {Promise<ScanResult[]>} */
  async getScanResults(listingId) {
    return (await this._get(this._scansKey(listingId))) || [];
  }

  /** @param {string} listingId @param {string} photoUrl @returns {Promise<ScanResult|null>} */
  async getScanForPhoto(listingId, photoUrl) {
    const scans = await this.getScanResults(listingId);
    return scans.find(s => s.photoUrl === photoUrl) || null;
  }

  /**
   * Save a scan result, replacing any previous result for the same photo.
   * @param {ScanResult} result
   */
  async addScanResult(result) {
    const scans = await this.getScanResults(result.listingId);
    const idx = scans.findIndex(s => s.photoUrl === result.photoUrl);
    if (idx >= 0) {
      scans[idx] = result;
    } else {
      scans.push(result);
    }
    await this._set(this._scansKey(result.listingId), scans);
  }
